    ...
    public partial class FormPersonViewerSCA :Form
    {
        ...
        private void GetFioFromServers(DataTable dataTablePeopple) //Get the list of registered users
        {
            ...
            try
            {
                ...
                stringConnection = @"server=" + mysqlServer + @";User=" + mysqlServerUserName + @";Password=" + mysqlServerUserPassword + @";database=wwwais;Allow Zero Datetime=true;Connect Timeout=60"; //convert zero datetime=True;
                logger.Info(stringConnection);
                        ...
                        using (var cmd = new MySql.Data.MySqlClient.MySqlCommand(query, sqlConnection))
                        {
                            using (MySql.Data.MySqlClient.MySqlDataReader reader = cmd.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    ...
                                    {
                                        MessageBox.Show(DateTime.Parse(reader?.GetMySqlDateTime(@"start_date").ToString()).ToString("yyyy-MM-dd HH:mm"));
